...
runnersLargeSizeShirt = runners.filter(function(sizes){
...
ticketPriceTotal = runners.reduce(function(acc, cur){
...
const runnerId = runners.filter(function(value){
  return value.id === 32;
});
...
const companysInfo = runners.forEach(function(param){
  // console.log(`${param.company_name} $${param.donation}`);
});
...
runners.forEach(function(param2){
  return nameEmail.push(`${param2.first_name}, ${param2.email}`);
});
